...
describe("listGames", () => {
  ...
  it("returns upcoming and past games on success", async () => {
    setupAdminChain();
    const upcomingGame = { id: "g1", location: "Quadra", scheduled_at: "2099-01-01T00:00:00Z", status: "open", draw_done: false, is_tournament: false };
    const pastGame = { id: "g2", location: null, scheduled_at: "2020-01-01T00:00:00Z", status: "finished", draw_done: true, is_tournament: false };
...
describe("createAndAddPlayer", () => {
  ...
  it("returns error when phone already exists in team", async () => {
    ...
    const result = await createAndAddPlayer("game-1", playerParams);
    expect(result).toEqual({ error: "Já existe um jogador com este telefone nesta turma." });
  });
