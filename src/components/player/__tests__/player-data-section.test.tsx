...
describe("PlayerDataSection", () => {
  ...
  it("renders stamina label for '1'", () => {
    render(<PlayerDataSection player={{ ...BASE_PLAYER, stamina: "1" }} teamId="team-1" />);
    expect(screen.getByText("1 jogo")).toBeInTheDocument();
  });

  it("renders stamina label for '4plus'", () => {
    render(<PlayerDataSection player={{ ...BASE_PLAYER, stamina: "4plus" }} teamId="team-1" />);
  ...
  it("shows star classification when is_star is true", () => {
    render(<PlayerDataSection player={{ ...BASE_PLAYER, is_star: true }} teamId="team-1" />);
  ...
  it("clicking 'Eu não sou esse jogador' calls clearPlayerCookie with teamId", async () => {
    ...
    await user.click(screen.getByRole("button", { name: /Eu não sou esse jogador/ }));
