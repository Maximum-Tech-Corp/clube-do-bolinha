...
describe("GameCard", () => {
  ...
  it("does not render location when null", () => {
    render(<GameCard {...BASE_PROPS} game={{ ...BASE_GAME, location: null }} />);
  ...
  it("shows 'Cancelado' badge for cancelled game", () => {
    render(<GameCard {...BASE_PROPS} game={{ ...BASE_GAME, status: "cancelled" }} />);
  ...
  it("shows 'Finalizado' badge for finished game", () => {
    render(<GameCard {...BASE_PROPS} game={{ ...BASE_GAME, status: "finished" }} />);
    expect(screen.getByText("Finalizado")).toBeInTheDocument();
  });

  it("shows 'Modo Campeonato' label when is_tournament is true", () => {
    render(<GameCard {...BASE_PROPS} game={{ ...BASE_GAME, is_tournament: true }} />);
  ...
  it("shows 'Ver lista' link when open and draw not done", () => {
    render(<GameCard {...BASE_PROPS} game={{ ...BASE_GAME, draw_done: false }} />);
  ...
  it("shows 'Acompanhar Jogos' when open, draw_done, is_tournament, tournamentStarted", () => {
    ...
    expect(link).toHaveAttribute("href", "/jogador/BOLA-ABC123/campeonato/game-1");
  });
  ...
  it("shows 'Ver times sorteados' when draw_done, is_tournament but tournament not started", () => {
    ...
    expect(screen.getByRole("link", { name: /Ver times sorteados/ })).toBeInTheDocument();
    expect(screen.queryByRole("link", { name: /Acompanhar Jogos/ })).not.toBeInTheDocument();
  });
  ...
  it("shows 'Ver detalhes' link when finished and detailsHref provided", () => {
    ...
    expect(link).toHaveAttribute("href", "/jogador/BOLA-ABC123/historico/game-1");
  });
