...
import { getDrawInfo, runDraw, redistributeStars, TEAM_SIZE } from "@/lib/draw-algorithm";
...
describe("getDrawInfo", () => {
  ...
  describe("canBeTournament", () => {
    it("3 teams → false", () => expect(getDrawInfo(15).canBeTournament).toBe(false));
    it("4 teams (20 players) → true", () => expect(getDrawInfo(20).canBeTournament).toBe(true));
    it("4 teams via partial (18 players) → true", () => expect(getDrawInfo(18).canBeTournament).toBe(true));
    it("5 teams (25 players) → true", () => expect(getDrawInfo(25).canBeTournament).toBe(true));
...
function makeScoredPlayer(id: string, isStar: boolean, score = 2): ScoredPlayer {
  return {
    id,
    name: id,
    weight_kg: 70,
